@extends('layouts.master')

@section('content')
<!-- Main -->
<div id="main">
	<section id="one" style="background-color: #fff;">
		<div class="inner">
			<header class="major">
				<h1 style="color: #000;">Contact Us</h1>
			</header>
			<!-- Content -->
			<p style="color: #000;">Get in touch with PSAC 77000.</p>
			
			<div style="color: #000;" class="table-wrapper">
			    <h3>Executive Committee</h3> 
			    <table style="color: #000;" class="alt">
			        <thead style="color: #000;">
			            <tr style="color: #000;">
			                <th style="color: #000;">Name</th>
			                <th style="color: #000;">Email</th>
			            </tr>
			        </thead>
			        <tbody>
		                <tr>
		                    <td style="color: #000;">Shermeen Nizami, interim President</td>
		                    <td style="color: #000;">president.psac77000@gmail.com</td>
		                </tr>
		                <tr>
		                    <td style="color: #000;">Jesse Heilman, interim Vice-President</td>
		                    <td style="color: #000;">vp.psac77000@gmail.com</td>
		                </tr>
		                <tr>
		                    <td style="color: #000;">Zied Bouida, interim Secretary-Treasurer</td>
		                    <td style="color: #000;">treasurer.psac77000@gmail.com</td>
		                </tr>
			        </tbody>
			    </table>
			    <h3>Communications Subcommittee</h3> 
			    <table style="color: #000;" class="alt">
			        <thead style="color: #000;">
			            <tr style="color: #000;">
			                <th style="color: #000;">Name</th>
			                <th style="color: #000;">Email</th>
			            </tr>
			        </thead>
			        <tbody> 
		                
		                <tr>
		                    <td style="color: #000;">Violeta Marquez-Cruz                    </td>
		                    						 
		                    <td style="color: #000;">contact.psac77000@gmail.com</td>
		                </tr>
		                <tr>
		                    <td style="color: #000;">Alberto Tonero</td>
		                    <td style="color: #000;">contact.psac77000@gmail.com</td>
		                </tr>
		                <tr>
		                    <td style="color: #000;">Zied Bouida</td>
		                    <td style="color: #000;">treasurer.psac77000@gmail.com</td>
		                </tr>
		                <tr>
		                    <td style="color: #000;">Anne-Marie Grondin</td>
		                    <td style="color: #000;">grondia@psac-afpc.com</td>
		                </tr>
			        </tbody>
			    </table>

			    <h3>Health & Safety Representative</h3>  
			    <table style="color: #000;" class="alt">
			        <thead style="color: #000;">
			            <tr style="color: #000;">
			                <th style="color: #000;">Name</th>
			                <th style="color: #000;">Email</th>
			            </tr>
			        </thead>
			        <tbody>
		                <tr>
		                    <td style="color: #000;">Chav Chhiv Chau</td>
		                    <td style="color: #000;">contact.psac77000@gmail.com</td>
		                </tr>
			        </tbody>
			    </table>
				
				<h3>Collective Bargaining Team</h3>
			    <table style="color: #000;" class="alt">
			        <thead style="color: #000;">
			            <tr style="color: #000;">
			                <th style="color: #000;">Name</th>
			                <th style="color: #000;">Email</th>
			            </tr>
			        </thead>
			        <tbody> 
		                <tr>
		                    <td style="color: #000;">Ashley Bickerton, PSAC Negotiator</td>
		                    <td style="color: #000;">bickera@psac-afpc.com </td>
		                </tr>
		                <tr>
		                    <td style="color: #000;">Jesse Heilman</td>
		                    <td style="color: #000;">contact.psac77000@gmail.com</td>
		                </tr>
		                <tr>
		                    <td style="color: #000;">Khaled Sebakhy</td>
		                    <td style="color: #000;">contact.psac77000@gmail.com</td>
		                </tr>
		                <tr>
		                    <td style="color: #000;">Chunyun Ma</td>
		                    <td style="color: #000;">contact.psac77000@gmail.com</td>
		                </tr>
		                <tr>
		                    <td style="color: #000;">Donald Fowler</td>
		                    <td style="color: #000;">contact.psac77000@gmail.com</td>
		                </tr>
			        </tbody>
			    </table>

			    <h3>PSAC-NCR</h3>  
			    <table style="color: #000;" class="alt">
			        <thead style="color: #000;">
			            <tr style="color: #000;">
			                <th style="color: #000;">Name</th>
			                <th style="color: #000;">Email</th>
			            </tr>
			        </thead>
			        <tbody> 
		                <tr>
		                    <td style="color: #000;">Anne-Marie Grondin, Regional Representative</td>
		                    <td style="color: #000;">grondia@psac-afpc.com</td>
		                </tr>
		                <tr>
		                    <td style="color: #000;">Greg McGillis, Regional Executive Vice-President</td>
		                    <td style="color: #000;">mcgillg@psac-afpc.com </td>
		                </tr>
			        </tbody>
			    </table>
			</div>

		</div>
	</section>

</div>

@endsection